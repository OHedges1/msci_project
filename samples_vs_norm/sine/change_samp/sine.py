...
class LagrangePoly:
    ...
    def get_2_norm(self):
        ...
        sin_vals = np.sin(pi*vals)
        interp_vals = self.interpolate(vals)
        return np.linalg.norm(interp_vals - sin_vals, 2)
...
def run_test(neurons, samples, epochs):
    ...
    sin_points = np.sin(pi*points)
    ax.plot(points, sin_points, '-', label='Sine function')


    # plot l-interpolation and get 2 norm
    samps = np.linspace(-1, 1, samples)
    sin_samps = np.sin(pi*samps)
    l_2norm = 0
    if samples <= 24:
        l = LagrangePoly(samps, sin_samps)
